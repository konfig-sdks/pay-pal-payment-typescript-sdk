...
import { readableStreamToString, PayPalPaymentError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PayPalPaymentError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PayPalPaymentError(e, e.response?.data)
                }
                if (innerError instanceof PayPalPaymentError) {
                    // Got "PayPalPaymentError" from the above try block
                    throw innerError;
                }
